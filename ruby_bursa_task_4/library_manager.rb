...
TEXT
        ...
        a = Hash.new
         @readers_with_books.each do |r|
          if r.reader.name == name
            ...
            a.store("penalty_per_hour", (r.amazing_book.penalty_per_hour()/100).round(1) )
            a.store("current_page", r.current_page)
            a.store("time_to_finish", r.time_to_finish.round(2))
            end
          end
          a
  end
  ...
  def librarian_notification_params
      ...
      { :number_of_books => "#{@books.count + @readers.count}", :number_of_readers => "#{@readers_with_books.count + @readers.count}", 
      :number_of_readers_with_book => "#{@readers_with_books.count}",
